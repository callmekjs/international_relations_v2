...
MIN_QUOTE_KEY = 12         # shorter quotes (after cite_key) prove nothing
...
_DIGIT_SEP_SENTINEL = chr(0xE001)
_DIGIT_SEP = re.compile('(?<=[0-9])[' + re.escape(_DASHES + chr(0xB7)) + '](?=[0-9])')
_KEY_TABLE: dict[int, str | None] = {ord(c): None for c in _DASHES + _QUOTES + "\u00b7"}
_KEY_TABLE.update({ord(c): "~" for c in _TILDES})
_KEY_TABLE.update({ord(_DIGIT_SEP_SENTINEL): "/"})
...
def cite_key(text: str) -> str:
    ...
    marked = _DIGIT_SEP.sub(_DIGIT_SEP_SENTINEL, _SIGN.sub(_SIGN_SENTINEL, _display(text)))
...
def keyed(text: str) -> tuple[str, str, list[int]]:
    ...
    disp = _display(text)
    marked = _DIGIT_SEP.sub(_DIGIT_SEP_SENTINEL, _SIGN.sub(_SIGN_SENTINEL, disp))
    chars: list[str] = []
    pos: list[int] = []
...
def find_bounded(kt: tuple[str, str, list[int]], qkey: str) -> int:
    """key.find(qkey), skipping matches that start or end inside a longer number in the DISPLAY text
    (47명 is not evidence for 7명). Line breaks/spaces between numbers count as a boundary."""
    disp, pkey, pos = kt
    j = pkey.find(qkey)
    while j >= 0:
        e = j + len(qkey)
        a, b = pos[j], pos[e - 1]
        before = disp[a - 1] if a > 0 else ""
        before2 = disp[a - 2] if a > 1 else ""
        after = disp[b + 1] if b + 1 < len(disp) else ""
        after2 = disp[b + 2] if b + 2 < len(disp) else ""
        cut_left = qkey[0].isdigit() and (before.isdigit() or (before in (".", ",") and before != "" and before2.isdigit()))
        cut_right = qkey[-1].isdigit() and (after.isdigit() or (after in (".", ",") and after != "" and after2.isdigit()))
        if not (cut_left or cut_right):
            return j
        j = pkey.find(qkey, j + 1)
    return -1
...
def _check(cit: dict, sentence: str, ctx: _Context, accept_near: bool, fix_wrong_page: bool) -> dict:
    ...
    edge = '.' + chr(0x2026) + chr(0x22EF) + chr(0x3002)
    qkey = cite_key(str(quote).strip().lstrip(edge).rstrip(edge + ',').strip())
    ...
    for other, k in ctx.keys.items():                   # quote is verbatim on another read page
        if other != pid and (j := find_bounded(k, qkey)) >= 0:
            r["found_on"] = other
            if fix_wrong_page:
                r["corrected"] = True
